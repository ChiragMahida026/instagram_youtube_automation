...
API.  Invoking the CLI makes it easy to leverage options like
`--fast-update` for incremental downloads【503652632033799†L146-L153】.  The
implementation groups media files by timestamp and exposes a simple
`PostInfo` data class for downstream processing.

**Note**: This module assumes that the `instaloader` executable is available on
your `PATH`.  Install it via pip (`pip install instaloader`) or system
package manager.  The CLI must have access to the internet to fetch
Instagram data.
"""
...
def _collect_posts(target_dir: Path) -> List[PostInfo]:
    ...
    for caption_file in sorted(target_dir.glob("*.txt")):
        ...
        media_files = [
            f for f in target_dir.glob(f"{base_name}*")
...
def download_posts(
    ...
    download_all: bool, optional
        ...
        Instaloader so it stops downloading at the first existing post【503652632033799†L146-L153】.  When
    ...
    if not download_all:
        cmd.append("--fast-update")  # incremental updates【503652632033799†L146-L153】
    # Save caption in .txt files (default) and avoid JSON/xz for simplicity
    cmd.extend([
        "--dirname-pattern", str(target_dir),
